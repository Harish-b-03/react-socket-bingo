...
socketIO.on("connection", (socket) => {
	socket.on("joinRoom", (userName, roomName) => {
		...
		if (!res.success) {
			...
			socket.emit("message", "Error while joining the room. Please try again");
			return;
		}
		socket.join(roomId);
	...
	socket.on("playerReadyToStart", () => {
		...
		if (res.success) {
			...
			socket.broadcast.to(resRoomId.data.roomId).emit("message", `${res.data.user.userName} is ready`);
		} else {
			socket.emit("readyConfirmed", res);
		}
	});
	...
	socket.on("startGame", () => {
		...
		if (res.success) {
			...
			if (resNextTurn.success) {
				...
				socket.nsp.to(roomId).emit("message", resNextTurn);
			}
		} else {
